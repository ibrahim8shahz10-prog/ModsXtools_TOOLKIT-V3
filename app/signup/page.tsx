    ...
    <div style={{ minHeight: '100vh', background: '#080810', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ width: '100%', maxWidth: '420px', position: 'relative', zIndex: 10 }}>
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          ...
          <h1 style={{ color: '#f1f5f9', fontSize: '24px', fontWeight: '800', letterSpacing: '-0.02em' }}>Create your account</h1>
          <p style={{ color: '#334155', fontSize: '14px', marginTop: '6px' }}>Join MODSxTOOLS — free forever</p>
        </div>

        <div className="card" style={{ padding: '28px' }}>
          <form onSubmit={handleSignup} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div>
              <label style={{ color: '#64748b', fontSize: '11px', fontWeight: '700', display: 'block', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Username</label>
              <input required value={username} onChange={e => setUsername(e.target.value)} className="input" placeholder="ibrahim_dev" />
            </div>
            <div>
            ...
            <div>
              ...
              <input required minLength={6} type="password" value={password} onChange={e => setPassword(e.target.value)} className="input" placeholder="••••••••" />
            </div>
        ...
        <p style={{ textAlign: 'center', color: '#1e293b', fontSize: '13px', marginTop: '20px' }}>
          ...
          <Link href="/login" style={{ color: '#ef4444', fontWeight: '600', textDecoration: 'none' }}>Log in</Link>
        </p>
      </div>
    </div>
  )
